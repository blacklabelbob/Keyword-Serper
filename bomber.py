...
async def get_keyword_data(input_keyword, input_country, api_key):
    print("Starting to get keyword data...")  # Debug statement
    # Get results
    ...
    write_keywords_to_csv(keyword_data)
...
def write_keywords_to_csv(keyword_data):
    directory = "Keyword Research"
    os.makedirs(directory, exist_ok=True)
    file_path = os.path.join(directory, "keyword_data.csv")

    with open(file_path, mode='w', newline='', encoding='utf-8') as file:
        writer = csv.writer(file)
        writer.writerow(["Category", "Keyword"])  # Header row

        for category, keywords in keyword_data.items():
            for keyword in keywords:
                writer.writerow([category, keyword])  # Write each keyword

async def suggestions_ai_analysis(keyword_data: str, api_key):
    print("Starting AI analysis...")  # Debug statement
    max_retries = 5
